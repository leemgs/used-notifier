...
> ⚠️ **당근 웹의 지역 검색은 봇 차단으로 자동 수집이 막혀 있습니다.**
> 지역을 반영하는 신형 경로(`/kr/buy-sell/s/?in=…`)는 결과가 클라이언트 렌더링 +
> **Proof-of-Work(PoW)** 로 보호돼 서버가 매물을 자동으로 읽을 수 없습니다(실측 확인).
> 파싱 가능한 기존 경로(`/kr/buy-sell/`)는 **비로그인 시 기본지역(서초4동)** 결과만 줍니다.
> 따라서 **비로그인 상태로는 내 동네(예: 매탄동) 당근 매물을 웹에서 자동으로 받을 수 없습니다.**
