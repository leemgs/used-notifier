...
>
> **특정 동네의 모든 무료 매물을 받을 때:** 물품명(키워드)을 `모두`, 희망 금액을 `0원`으로
> 두면 제품명으로 제한하지 않고 모든 무료 매물을 알립니다. `모두`는 내부적으로 당근의 `나눔`
> 검색으로 변환됩니다. UI의 **모든 물건 알림** 또는 설정 파일의 `"allItems": true`도 같은
> 기능입니다.
>
> **지역 단위(시/구/동)에 맞게 조회합니다.** 당근의 `in=` 지역 코드는 동 단위뿐 아니라
> **구 단위**(예: `수원시-영통구-1293`)·**시 단위**(예: `수원시-4179`) 코드도 지원합니다.
> `location`에 `수원시`처럼 시를 지정하면 시 전역 코드와 확인된 구 코드를 함께 조회하고,
> `수원시영통구`처럼 구를 지정하면 구 전역 코드로 자동 조회하므로 **선택한 지역 전체**의 매물이
> 후보에 들어옵니다. 무료 전체 감시는 `나눔`과 `무료` 검색 결과도 합쳐 검색 결과 상한으로 인한
> 누락을 줄입니다. (예전에는 시/구를
> 골라도 대표 동 하나로만 조회해 그 동네 매물만 나왔습니다.) 특정 동만 보려면 `"location":
> "매탄동"`, `"daangnRegion": "매탄동-4535"`처럼 동 코드를 직접 지정하세요. `daangnRegion`에
> 여러 코드를 **콤마로** 넣으면(`"매탄동-4535, 영통동-4537"`) 각각 조회해 병합합니다. 이
> 저장소에는 **수원시 전역** 무료나눔 감시 항목이 기본 수신 이메일로 활성화되어 있습니다.
>
> **나눔 등록일 범위:** 무료나눔을 선택하면 **며칠 전까지** 입력란이 나타납니다. `0`은 오늘,
> `3`은 최근 3일 이내 등록된 매물만 이메일과 이슈로 알립니다. 비워두면 등록일 제한이 없으며,
> 범위를 지정했지만 사이트에서 등록일을 확인할 수 없는 매물은 오래된 매물 오알림을 막기 위해 제외합니다.
...
당근 웹 검색은 **지역을 지정하지 않으면 기본 지역(서초4동)** 결과만 줍니다. `location`에 시/구/동을
지정하면 그 단위에 맞는 코드로 **자동 조회**합니다(시→시 전역, 구→구 전역). 특정 동만 좁혀서
보거나 자동 코드가 없는 지역은 `daangnRegion`에 당근 검색 URL의 `in=` 값을 직접 넣으세요(**로그인 불필요**).

1. 브라우저에서 [daangn.com 검색](https://www.daangn.com/kr/buy-sell/s/) 을 열고 왼쪽 위 **지역**을 원하는 시/구/동으로 변경
2. 아무 키워드나 검색한 뒤 주소창 URL의 `in=` 값을 복사 — 예: 동 `…?in=매탄동-4535…`, 구 `…?in=수원시-영통구-1293…`, 시 `…?in=수원시-4179…`
3. 감시 항목에 `"daangnRegion": "매탄동-4535"` 로 저장 (콤마로 여러 코드 지정 시 각각 조회해 병합: `"매탄동-4535, 영통동-4537"`)

> ⚠️ **정확한 지역 코드**가 중요합니다 — 코드가 틀리면 기본 지역(서초4동)으로 되돌아갑니다.
> 검색기는 파싱 가능한 `/kr/buy-sell/?search=…&in=<코드>` 경로로 그 지역을 직접 조회합니다.
> (신형 `/kr/buy-sell/s/` 경로는 결과가 PoW 봇 차단으로 보호돼 사용하지 않습니다.)
>
> 💡 `in=` 코드는 **시/구/동 단위** 모두 있습니다. 시/구 전체를 받고 싶으면 대표 동 코드 대신
> 시·구 코드를 쓰세요 — 동 코드 하나로 조회하면 그 동네 반경 매물만 나옵니다.
...
쿠키나 `in=`을 사용하더라도 응답의 실제 매물 지역은 감시 지역과 다시 대조합니다.
당근이 기본 지역으로 되돌아간 경우 다른 지역 매물을 알리지 않도록 일치하지 않는 결과는 제외합니다.
...
#### ⚠️ 이메일 발송 실패 시 자동 이슈 등록
Gmail SMTP 로 이메일 발송이 실패하면(예: `GMAIL_APP_PASSWORD` 미등록·오타·앱 비밀번호 만료),
그 사실을 **`이메일-발송실패` 라벨의 GitHub 이슈로 자동 등록**합니다. 이슈에는 발생 시각,
영향받은 감시 항목/사이트, 오류 메시지, 점검 체크리스트가 담깁니다(수신 이메일은 마스킹).

- 워크플로가 ~3분마다 반복 실행되므로, **같은 실패 이슈가 열려 있는 동안에는 중복 생성하지
  않습니다.** 원인을 해결한 뒤 이슈를 직접 닫으세요(자동으로 닫히지는 않습니다).
- `GITHUB_TOKEN` 만 있으면 동작하며 별도 설정이 필요 없습니다.
